...
export default function OfficerCaseDetailPage() {
  ...
  async function submitNote() {
    ...
    setAddingNote(true);
    try {
      await api.addCaseNote(id, noteText, true);
  ...
  const { person, milestones, caseNotes, applicationTracking, orders } = engagement;
          ...
          <Card className="p-5">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto mb-3">
              {caseNotes.length === 0 ? (
                ...
                caseNotes.map((n: any) => (
                  <div key={n.id} className="rounded-xl bg-muted/50 px-3 py-2 text-xs">
            ...
            <button
              ...
              className="mt-2 w-full rounded-xl bg-primary px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary/90 disabled:opacity-50 transition-colors"
            >
